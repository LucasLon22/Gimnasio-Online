...
const ListasDeUsuarios = () => {
    ...
    const borrarUsuario = (codigo) => {
        axios.post('/api/usuario/borrarusuario', { codigo: codigo }).then(res => {
            console.log(res.data)
            // alert(res.data)
    ...
    const listausuarios = dataUsuarios.map(usuario => {
        return (
                    ...
                    <td><div className='text-center'>
                        <Link to={`/editarusuario/${usuario.codigo}`} className='hipervinculos-sin-azul'><FaPen className='fs-5 me-3 text-dark' /></Link>
                        <FaTrash className='ms-3 fs-5' type='button' onClick={() => { borrarUsuario(usuario.codigo) }} /></div></td>
                        </tr>
                    </tbody>
        )
    })
